...
int CertificateManager::apply_dead_rule(int setid, std::string rulename,
...
int CertificateManager::apply_subset_rule(int left_setid, int right_setid, std::string rulename,
                                            std::vector<int> justification) {
    int new_kid = get_new_knowledgeid();
...
int CertificateManager::get_new_setid() {
    return setcount++;
}
int CertificateManager::get_new_knowledgeid() {
    return knowledgecount++;
}

SetExpression CertificateManager::get_emptyset() {
    if (emptyset.id == -1) {
...
SetExpression CertificateManager::get_goalset() {
    if (goalset.id == -1) {
        int new_sid = get_new_setid();
...
SetExpression CertificateManager::get_initset() {
    if (initset.id == -1) {
        int new_sid = get_new_setid();
...
SetExpression CertificateManager::define_set_negation(SetExpression &set) {
    int new_sid = get_new_setid();
    certstream << "e " << new_sid << " n " << set.id << "\n";
    return SetExpression(new_sid);
}
SetExpression CertificateManager::define_set_union(SetExpression &left_set, SetExpression &right_set) {
    int new_sid = get_new_setid();
    certstream << "e " << new_sid << " u " << left_set.id << " " << right_set.id << "\n";
    return SetExpression(new_sid);
}
SetExpression CertificateManager::define_set_intersection(SetExpression &left_set, SetExpression &right_set) {
    int new_sid = get_new_setid();
    certstream << "e " << new_sid << " i " << left_set.id << " " << right_set.id << " \n";
    return SetExpression(new_sid);
}
SetExpression CertificateManager::define_set_progression(SetExpression &set, int actionset_id) {
    int new_sid = get_new_setid();
    certstream << "e " << new_sid << " p " << set.id << " " << actionset_id << "\n";
    return SetExpression(new_sid);
}
SetExpression CertificateManager::define_set_regression(SetExpression &set, int actionset_id) {
    int new_sid = get_new_setid();
    certstream << "e " << new_sid << " r " << set.id << " " << actionset_id << "\n";
    return SetExpression(new_sid);
}

Judgment CertificateManager::make_statement(SetExpression left_set, SetExpression right_set, std::string type) {
    int new_kid = get_new_knowledgeid();
...
Judgment CertificateManager::apply_rule_ud(SetExpression set, Judgment &s_dead,
                                             Judgment &sp_dead) {
    return Judgment(apply_dead_rule(set.id, "ud", {s_dead.id, sp_dead.id}));
}
Judgment CertificateManager::apply_rule_sd(SetExpression set, Judgment &sp_dead,
                                             Judgment &s_subset_sp) {
    return Judgment(apply_dead_rule(set.id, "sd", {sp_dead.id, s_subset_sp.id}));
}
Judgment CertificateManager::apply_rule_pg(SetExpression set, Judgment &progression_subset,
                                             Judgment &sp_dead,
...
Judgment CertificateManager::apply_rule_pi(SetExpression set, Judgment &progression_subset,
                                             Judgment &sp_dead,
                                             Judgment &init_subset) {
    return Judgment(apply_dead_rule(set.id, "pi", {progression_subset.id, sp_dead.id, init_subset.id}));
}
Judgment CertificateManager::apply_rule_rg(SetExpression set, Judgment &regression_subset,
                                             Judgment &sp_dead,
                                             Judgment &goal_intersection_dead) {
    return Judgment(apply_dead_rule(set.id, "rg", {regression_subset.id, sp_dead.id, goal_intersection_dead.id}));
}
Judgment CertificateManager::apply_rule_ri(SetExpression set, Judgment &regression_subset,
                                             Judgment &sp_dead,
                                             Judgment &init_subset) {
    return Judgment(apply_dead_rule(set.id, "ri", {regression_subset.id, sp_dead.id, init_subset.id}));
}

Judgment CertificateManager::apply_rule_ci(Judgment &init_dead) {
    int new_kid = get_new_knowledgeid();
    certstream << "k " << new_kid << " u ci " << init_dead.id << "\n";
    return Judgment(new_kid);
}
Judgment CertificateManager::apply_rule_cg(Judgment &goal_dead) {
    int new_kid = get_new_knowledgeid();
    certstream << "k " << new_kid << "u cg " << goal_dead.id << "\n";
    return Judgment(new_kid);
}

Judgment CertificateManager::apply_rule_ur(SetExpression left_set, SetExpression right_set) {
    return Judgment(apply_subset_rule(left_set.id, right_set.id, "ur", {}));
}
Judgment CertificateManager::apply_rule_ul(SetExpression left_set, SetExpression right_set) {
    return Judgment(apply_subset_rule(left_set.id, right_set.id, "ul", {}));
}
Judgment CertificateManager::apply_rule_ir(SetExpression left_set, SetExpression right_set) {
    return Judgment(apply_subset_rule(left_set.id, right_set.id, "ir", {}));
}
Judgment CertificateManager::apply_rule_il(SetExpression left_set, SetExpression right_set) {
    return Judgment(apply_subset_rule(left_set.id, right_set.id, "il", {}));
}
Judgment CertificateManager::apply_rule_di(SetExpression left_set, SetExpression right_set) {
    return Judgment(apply_subset_rule(left_set.id, right_set.id, "di", {}));
}
Judgment CertificateManager::apply_rule_su(SetExpression left_set, SetExpression right_set,
...
Judgment CertificateManager::apply_rule_si(SetExpression left_set, SetExpression right_set,
                                             Judgment &e_subset_ep,
                                             Judgment &e_subset_epp) {
    return Judgment(apply_subset_rule(left_set.id, right_set.id, "si", {e_subset_ep.id, e_subset_epp.id}));
}
Judgment CertificateManager::apply_rule_st(SetExpression left_set, SetExpression right_set,
                                             Judgment &e_subset_ep,
                                             Judgment &ep_subset_epp) {
    return Judgment(apply_subset_rule(left_set.id, right_set.id, "st", {e_subset_ep.id, ep_subset_epp.id}));
}

Judgment CertificateManager::apply_rule_at(SetExpression left_set, SetExpression right_set,
                                             Judgment &prog_subset,
                                             Judgment &a_subset) {
    return Judgment(apply_subset_rule(left_set.id, right_set.id, "at", {prog_subset.id, a_subset.id}));
}
Judgment CertificateManager::apply_rule_au(SetExpression left_set, SetExpression right_set,
...
Judgment CertificateManager::apply_rule_pt(SetExpression left_set, SetExpression right_set,
                                             Judgment &prog_subset,
                                             Judgment &sp_subset_s) {
    return Judgment(apply_subset_rule(left_set.id, right_set.id, "pt", {prog_subset.id, sp_subset_s.id}));
}
Judgment CertificateManager::apply_rule_pu(SetExpression left_set, SetExpression right_set,
...
Judgment CertificateManager::apply_rule_pr(SetExpression left_set, SetExpression right_set,
                                             Judgment &progression) {
    return Judgment(apply_subset_rule(left_set.id, right_set.id, "pr", {progression.id}));
}
Judgment CertificateManager::apply_rule_rp(SetExpression left_set, SetExpression right_set,
                                             Judgment &regression) {
